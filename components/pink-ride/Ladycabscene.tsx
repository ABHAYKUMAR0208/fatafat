...
const cabImg = require('../../assets/illustrations/lady-cab.png');
const CAB_ASPECT = 582 / 300;
...
function Cab({ sceneWidth, roadHeight }: { sceneWidth: number; roadHeight: number }) {
  ...
  const width = Math.min(sceneWidth * 0.78, 300);
  const height = width / CAB_ASPECT;
  const puddleWidth = width * 0.7;

  return (
    <View style={[styles.riderWrap, { bottom: roadHeight * GROUND_OFFSET_PCT }]} pointerEvents="none">
      <Animated.View
        style={[styles.puddle, { width: puddleWidth, left: '50%', marginLeft: -puddleWidth / 2 }, puddleStyle]}
      />
      <Animated.View style={style}>
        <Image source={cabImg} style={{ width, height, transform: [{ scaleX: 1 }] }} resizeMode="contain" />
      </Animated.View>
    </View>
  );
}
...
function SpeedLines({ sceneWidth, roadHeight }: { sceneWidth: number; roadHeight: number }) {
  ...
  return (
    <View
      style={[styles.speedLines, { bottom: roadHeight * 0.65 + 46, left: sceneWidth * 0.04 }]}
      pointerEvents="none"
    >
      {bases.map((delay, i) => (
...
function SpeedDash({ top, width, delay }: { top: number; width: number; delay: number }) {
  ...
  const style = useAnimatedStyle(() => ({
    ...
    transform: [{ translateX: -20 + t.value * 60 }],
  }));
...
export default function LadyCabScene() {
  ...
  return (
    ...
    >
      <View style={styles.tag}>
        <Text style={styles.tagText}>LADY CAB</Text>
      </View>
      ...
      {size.width > 0 && (
        <>
          ...
          <Sparkle top={size.height * 0.4} left={size.width * 0.82} delay={800} />
          <Sparkle top={size.height * 0.6} left={size.width * 0.18} delay={1400} />
          ...
          <Cab sceneWidth={size.width} roadHeight={size.height * ROAD_HEIGHT_PCT} />
          <Road sceneWidth={size.width} sceneHeight={size.height} />
        </>
      )}

      <Text style={styles.caption}>Verified lady captain · every trip</Text>
    </View>
  );
}
