...
/**
 * Real 3D hero illustration for web builds.
 * Requires: three, @react-three/fiber  (no expo-gl needed on web —
 * @react-three/fiber's default export renders to a normal HTML <canvas>,
 * which works fine since Expo Web runs on react-dom under the hood.)
 *
 * Metro resolves this file automatically for web (via the `.web.tsx` suffix);
 * native (iOS/Android) builds instead get `Carpool3DScene.native.tsx`, which
 * uses `@react-three/fiber/native` + `expo-gl`. Keep the two scenes visually
...
const styles = StyleSheet.create({
  ...
  // @ts-ignore -- web canvas element accepts plain CSS, no-op type mismatch is fine here
